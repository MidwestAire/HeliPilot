...
bool AP_Arming_Copter::run_pre_arm_checks(bool display_failure)
{
    ...
    if (copter.ap.using_interlock && copter.ap.motor_interlock_switch) {
        check_failed(display_failure, "Check Throttle Position");
    }
...
bool AP_Arming_Copter::ins_checks(bool display_failure)
{
    ...
    if ((checks_to_perform == ARMING_CHECK_ALL) || (checks_to_perform & ARMING_CHECK_INS)) {
        ...
        if (!pre_arm_ekf_attitude_check()) {
            check_failed(ARMING_CHECK_INS, display_failure, "gyro bias excessive");
            ret = false;
        }
    }
...
bool AP_Arming_Copter::parameter_checks(bool display_failure)
{
    ...
    if ((checks_to_perform == ARMING_CHECK_ALL) || (checks_to_perform & ARMING_CHECK_PARAMETERS)) {
        ...
        #if FRAME_CONFIG == HELI_FRAME
        if (copter.g2.frame_class.get() != AP_Motors::MOTOR_FRAME_HELI) {
        ...
        if (rc().find_channel_for_option(RC_Channel::aux_func_t::MOTOR_INTERLOCK) == nullptr) {
            check_failed(ARMING_CHECK_PARAMETERS, display_failure, "Heli Throttle not configured");
            return false;
        }
...
bool AP_Arming_Copter::arm_checks(AP_Arming::Method method)
{
    ...
    if (copter.ap.using_interlock && copter.ap.motor_interlock_switch) {
        check_failed(true, "Check Throttle Position");
        return false;
    }
